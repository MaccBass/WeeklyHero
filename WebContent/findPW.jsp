...
<center>
	...
	<br><br><br>
	<%
	if(user.getUserID()==null || user.getUserE_mail()==null){
		...
		if(result==""){
			PrintWriter script=response.getWriter();
			script.println("<script>");
			script.println("alert('해당 정보로 가입된 비밀번호가 없습니다.')");
			script.println("history.back()");
			script.println("</script>");
		} else{
			session.setAttribute("foundPW", result);
		}
	}
	
	out.println("<h3>회원님의 비밀번호는</h3><h1> "+session.getAttribute("foundPW")+"</h1><h3> 입니다.</h3>");
	%>
	<br><br><br>
